...
export const db = {
  ...
  async saveDailyForm(form: {
    ...
    ferramentas: string[];
    tempo_medio_por_atividade: string;
    maior_consumo_tempo: string;
    impedimentos: string;
  }) {
  ...
  async saveProcess(form: {
    ...
    frequencia: string;
    tempo_medio: string;
    depende_outros: boolean;
    setor_dependencia?: string;
    pode_automatizar: boolean;
    como_automatizar?: string;
  }) {
    const { error } = await supabase.from("repetitive_processes").insert(form);
    if (error) throw error;
  },
  ...
  async saveBottleneck(form: {
    ...
    const { error } = await supabase.from("bottlenecks").insert(form);
    if (error) throw error;
  },
  ...
  async saveToolMapping(form: {
    ...
    tempo_gasto_semana: string;
    compartilha_com: string;
    setores_envolvidos: string[];
    problemas: string;
    satisfacao: string;
    gostaria_substituir: boolean;
    como_seria_ideal?: string;
    criado_por_voce: boolean;
    quantas_pessoas_usam: string;
  }) {
